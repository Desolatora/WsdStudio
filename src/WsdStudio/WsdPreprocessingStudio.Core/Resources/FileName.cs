namespace WsdPreprocessingStudio.Core.Resources
{
    internal static class FileName
    {
        public const string Dictionary = "dictionary";
        public const string WordEmbeddings = "word-embeddings";
        public const string MeaningEmbeddings = "meaning-embeddings";
        public const string DataAnalysis = "data-analysis";
        public const string DictionaryStatistics = "dictionary-statistics";
        public const string DataStatistics = "data-statistics";
        public const string WordEmbeddingsStatistics = "word-embeddings-statistics";
        public const string MeaningEmbeddingsStatistics = "meaning-embeddings-statistics";
    }
}
